...
# CONFIGURACI├ôN DEL MODELO
# ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
SEASON        = '2025-26'
MIN_GAMES     = 8        # M├¡nimo de partidos para que el an├ílisis sea v├ílido
MIN_PROB      = 0.90     # Probabilidad estad├¡stica m├¡nima real
MIN_EDGE      = 0.05     # Edge m├¡nimo sobre la prob. impl├¡cita sin vig
MIN_MINUTES   = 18.0     # Descartar jugadores con < 18 min promedio (garbage time)
WEIGHTS       = (0.50, 0.30, 0.20)  # Temporada / ├Ültimos 10 / ├Ültimos 5
# L├¡nea m├¡nima como % de la media proyectada (filtra l├¡neas ex├│ticas/alternativas muy bajas)
MIN_LINE_RATIO = 0.60    # La l├¡nea debe ser al menos el 60% de la media del jugador
...
class EVAnalyzer:
    def __init__(self):
        ...
        # Caches precargados al inicio (una sola llamada por categor├¡a)
        self._usage_cache = {}   # player_name ÔåÆ usage_rate (float)
        self._pace_cache  = {}   # team_name   ÔåÆ pace (float)
    ...
    def preload_league_stats(self):
        """
        Descarga en UNA sola llamada las estad├¡sticas avanzadas de TODA la liga:
        - Usage Rate de todos los jugadores
        - Pace de todos los equipos
        Esto es mucho m├ís eficiente que una llamada por jugador.
        """
        from nba_api.stats.endpoints import leaguedashplayerstats, leaguedashteamstats

        # ÔöÇÔöÇ Usage Rate de jugadores ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        ...
        try:
            ...
            logger.info(f"  ÔåÆ Usage cargado para {len(self._usage_cache)} jugadores.")
        except Exception as e:
            logger.warning(f"  ÔåÆ No se pudo cargar Usage Rate: {e}. Se usar├í 20% por defecto.")

        # ÔöÇÔöÇ Pace de equipos ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        ...
        try:
            ...
            logger.info(f"  ÔåÆ Pace cargado para {len(self._pace_cache)} equipos.")
        except Exception as e:
            logger.warning(f"  ÔåÆ No se pudo cargar Pace: {e}. Se usar├í 100 por defecto.")

    def preload_injury_report(self):
        """
        Descarga el injury report directamente del CDN p├║blico de NBA.com.
        ...
        try:
            ...
            logger.info(f"  ÔåÆ {len(self._injured)} jugadores Out/Doubtful en injury report.")
        except Exception as e:
            logger.warning(f"  ÔåÆ Injury report no disponible: {e}. No se aplicar├í filtro de lesiones.")
    ...
    def get_todays_odds(self):
        """Descarga las l├¡neas de apuestas activas desde Supabase."""
        ...
        try:
            ...
            data = response.data
            logger.info(f"  ÔåÆ {len(data)} l├¡neas encontradas.")
            return data
        except Exception as e:
    ...
    def get_player_gamelog(self, player_name: str):
        ...
        try:
            ...
            if df['MIN'].mean() < MIN_MINUTES:
                logger.info(f"    ÔåÆ {player_name} promedia {df['MIN'].mean():.1f} min. Ignorado.")
                return None
            return df
        except Exception as e:
    ...
    def remove_vig(over_odds: float, under_odds: float) -> tuple:
        ...
        Retorna las probabilidades reales impl├¡citas sin vig.

        Si solo tenemos una cuota, asumimos vig est├índar del 6%.
        """
        if over_odds and under_odds and over_odds > 0 and under_odds > 0:
            # M├®todo de normalizaci├│n bilateral (m├ís preciso)
    ...
    def is_back_to_back(season_df: pd.DataFrame) -> bool:
        """True si el ├║ltimo partido registrado fue ayer (B2B)."""
        try:
    ...
    def projected_stat(self, season_df: pd.DataFrame, stat_col: str,
        ...
        """
        Proyecci├│n ponderada del stat con todos los factores contextuales.
        ...
        b2b_penalty  = 0.92 if b2b else 1.0  # -8% en B2B (dato emp├¡rico NBA)
        ...
        # Consistencia: 100 = extremadamente consistente, 0 = muy err├ítico
    ...
    def calculate_ev(self):
        ...
        for name in entities:
            ...
            # ÔöÇÔöÇ EQUIPO (Moneyline) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
            if is_team:
                ...
                for odds in entity_odds:
                    ...
                    # Probabilidad impl├¡cita sin vig
            ...
            # ÔöÇÔöÇ JUGADOR ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
            logger.info(f"  [JUGADOR] {name}")

            # Filtro de lesiones
            if name in self._injured:
                logger.info(f"    ÔåÆ {name} est├í en injury report (Out/Doubtful). Ignorado.")
                continue
            ...
            if b2b:
                logger.info(f"    ÔåÆ B2B detectado para {name}.")
            ...
            for odds in entity_odds:
                ...
                # Proyecci├│n con todos los factores
                proj_mean, std_dev, median, consistency = self.projected_stat(
                    season_df, stat_col, usage, pace, b2b
                )

                # ÔöÇÔöÇ Filtro de l├¡neas ex├│ticas/alternativas muy bajas ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                # Si la l├¡nea es menor al 60% del promedio proyectado,
                # es casi imposible de NO superar ÔåÆ dato in├║til estad├¡sticamente.
                if proj_mean > 0 and (line / proj_mean) < MIN_LINE_RATIO:
                    logger.info(f"    ÔåÆ L├¡nea {line} muy baja vs media {proj_mean:.1f} para {name}. Ignorada.")
                    continue

                # Probabilidades sin vig (comparaci├│n justa)
                ...
                # ÔöÇÔöÇ OVER ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
                ...
                # ÔöÇÔöÇ UNDER ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        ...
        if ev_bets:
            ...
            # Un solo resultado por jugador+mercado (la mejor l├¡nea matem├íticamente)
    ...
    def send_telegram_alert(self, ev_bets: list):
        """Env├¡a todas las apuestas del d├¡a a Telegram (auto-dividido si supera 4000 chars)."""
        import requests
        token   = os.getenv("TELEGRAM_BOT_TOKEN")
        chat_id = os.getenv("TELEGRAM_CHAT_ID")
        if not token or not chat_id:
        ...
        msg  = "­ƒñû <b>MODELO PRO v2 ÔÇö APUESTAS +EV DEL D├ìA</b> ­ƒñû\n"
        msg += f"<i>Filtros: ProbÔëÑ{int(MIN_PROB*100)}% | EdgeÔëÑ{int(MIN_EDGE*100)}% | MinÔëÑ{MIN_MINUTES}min | Vig eliminado | Lesiones excluidas</i>\n\n"

        for idx, bet in enumerate(ev_bets, 1):
            b2b_tag = "ÔÜá´©Å <b>B2B</b> " if "B2B:True" in bet.get("info", "") else ""
            inj_tag = ""
            text  = f"<b>{idx}. {bet['player/team']}</b> {b2b_tag}{inj_tag}\n"
            text += f"­ƒÅƒ´©Å {bet.get('matchup','?')} ÔÇö {bet.get('game_time','?')}\n"
            text += f"­ƒÄ» <b>{bet['bet']}</b> ({bet['market']})\n"
            text += f"­ƒÆ░ Cuota: <b>{bet['odds']}</b> en {bet['bookie']}\n"
            text += f"­ƒôè Prob Real: <b>{bet['prob_real']}</b> | Sin vig casino: {bet['prob_casino_novig']}\n"
            text += f"­ƒôê Edge (sin vig): <b>{bet['edge']}</b>\n"
            text += f"­ƒö¼ {bet.get('info','')}\n\n"
